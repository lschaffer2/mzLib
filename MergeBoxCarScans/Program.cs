...
namespace BoxCar
{
    ...
    public static class Program
    {
        ...
        {
            Console.WriteLine("Load raw file.");
            var filepath = Console.ReadLine();
            var rawFile = ThermoStaticData.LoadAllStaticData(filepath);

            var list = SeparateScans(rawFile);
            var fullMs = list.First().Ms1scans.First();
            var boxcar1 = list.First().BoxcarScans[0];
            var boxcar2 = list.First().BoxcarScans[1];
            var boxcar3 = list.First().BoxcarScans[2];


            List<double> commonMzGrid = new List<double>();
            double themz = boxcar1.ScanWindowRange.Minimum;
            commonMzGrid.Add(themz);
            while (themz < boxcar1.ScanWindowRange.Maximum) //don't do last m/z window of 10
            {
                List<double> medianMzDifferences = new List<double>();
                foreach (var scan in list.First().Ms1scans.Concat(list.First().BoxcarScans))
                {
                    var listXArray = scan.MassSpectrum.XArray.ToList();
                    double closest =
                        scan.MassSpectrum.XArray.Aggregate((x, y) => Math.Abs(x - themz) < Math.Abs(y - themz) ? x : y);
                    int closestIndex = listXArray.IndexOf(closest);
                    if (closestIndex > listXArray.Count - 11) continue;
                    double[] mzDifferencesInScan = new double[10];
                    for (int i = 0; i < 10; i++)
                    {
                        int index1 = closestIndex + i;
                        int index2 = closestIndex + i + 1;
                        mzDifferencesInScan[i] = Math.Abs(listXArray[index1] - listXArray[index2]);
                    }

                    double medianMzDifference = mzDifferencesInScan.Median();
                    medianMzDifferences.Add(medianMzDifference);
                }

                double averageMzDifference = medianMzDifferences.Average();
                for (int i = 0; i < 10; i++)
                {
                    themz += averageMzDifference;
                    commonMzGrid.Add(themz);
                    Console.WriteLine(themz);
                }
            }

            using (var writer = new StreamWriter("C:\\users\\lschaffer2\\desktop\\mzcommongrid.tsv"))
            {
                foreach(var x in commonMzGrid) writer.WriteLine(x);
            }
